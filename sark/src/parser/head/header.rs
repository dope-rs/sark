use sark_core::error::{Error, Result};
use sark_core::http::codec;
use sark_core::http::head::{
    ERR_INVALID_HEADER_NAME, Flags, HeadInput, HeaderLine, HeaderLineScan, KnownHeader,
    is_header_name_byte,
};
...
impl HeaderApply {
        ...
        P: HeadParts<K>,
    {
        ...
        if let Some(start) = pretrim_start {
            ...
            while value_start < line.len() && HeaderLine::is_whitespace(line[value_start]) {
                value_start += 1;
            }
            while value_end > value_start && HeaderLine::is_whitespace(line[value_end - 1]) {
                value_end -= 1;
            }
        }
