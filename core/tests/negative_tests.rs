...
fn parse_cl_absent() {
    let headers: &[httparse::Header<'_>] = &[];
    assert_eq!(Parse::content_length(headers).unwrap(), None);
}
...
fn parse_cl_valid() {
    ...
    assert_eq!(Parse::content_length(headers).unwrap(), Some(123));
}
...
fn parse_cl_zero() {
    ...
    assert_eq!(Parse::content_length(headers).unwrap(), Some(0));
}
...
fn parse_cl_returns_first_match() {
    ...
    assert_eq!(Parse::content_length(headers).unwrap(), Some(10));
}
...
fn parse_cl_skips_other_headers() {
    ...
    assert_eq!(Parse::content_length(headers).unwrap(), Some(77));
}
...
fn is_chunked_true() {
    ...
    assert!(Parse::is_chunked(headers));
}
...
fn decode_head_is_head_request() {
    let raw = b"HTTP/1.1 200 OK\r\nContent-Length: 5\r\n\r\nhello";
    let head = Parse::head(raw, sark_core::http::codec::DecodeMode::Head)
        .unwrap()
        .unwrap();
    assert!(matches!(head.body_kind, BodyKind::NoBody));
}
...
fn decode_head_partial() {
    let raw = b"HTTP/1.1 200 OK\r\n";
    assert!(
        Parse::head(raw, sark_core::http::codec::DecodeMode::Response)
            .unwrap()
            .is_none()
    );
}
...
fn decode_head_empty_returns_none() {
    assert!(
        Parse::head(b"", sark_core::http::codec::DecodeMode::Response)
            .unwrap()
            .is_none()
    );
}
...
fn chunked_body_single_chunk() {
    let data = b"5\r\nhello\r\n0\r\n\r\n";
    let body = Parse::chunked_body(data).unwrap().unwrap();
...
fn chunked_body_partial_returns_none() {
    let data = b"5\r\nhel";
    assert!(Parse::chunked_body(data).unwrap().is_none());
}

#[test]
fn chunked_body_invalid_hex() {
    let data = b"xyz\r\nhello\r\n0\r\n\r\n";
    assert!(Parse::chunked_body(data).is_err());
}
...
fn response_binary_body() {
    ...
    let resp = Parse::response(&buf, sark_core::http::codec::DecodeMode::Response)
        .unwrap()
        .unwrap();
...
fn decode_after_eof_empty_buffer() {
    let result = Parse::response_after_eof(b"");
    assert!(result.is_err());
}

#[test]
fn decode_after_eof_just_status_line() {
    let result = Parse::response_after_eof(b"HTTP/1.1 200 OK\r\n\r\n");
    assert!(result.is_ok());
